...
@Slf4j
@SpringBootTest
public class RedisFlashSaleTest {
    ...
    @Test
    public void testRedisAtomicFlashSale() throws InterruptedException {
        log.info("=== Redis原子操作秒杀测试开始 ===");
     ...
     * 执行Redis秒杀核心逻辑
     */
    private void executeRedisFlashSale(int userId) {
        try {
            String stockKey = "flash:stock:" + TEST_ITEM_ID;
            String recordKey = "flash:user:record:" + userId + ":" + TEST_ITEM_ID;
            ...
            // 3. 记录用户参与，设置24小时过期
            redisUtil.set(recordKey, userId, 86400);
    ...
    private void generateTestReport(long duration) {
        ...
        log.info("其他失败: {}", failCount.get() - duplicateCount.get() - soldOutCount.get());
     ...
     */
    @Test
    public void testSimpleHighConcurrency() throws InterruptedException {
        log.info("=== 简化版高并发测试 ===");
        ...
        Object finalStock = redisUtil.get(stockKey);
        int remainingStock = finalStock != null ? Integer.parseInt(finalStock.toString()) : 0;
        log.info("最终库存: {}, 成功秒杀: {}", remainingStock, successCount.get());
        log.info("库存一致性: {}", (HIGH_INITIAL_STOCK - remainingStock) == successCount.get() ? "✅ 通过" : "❌ 失败");
    }
}
